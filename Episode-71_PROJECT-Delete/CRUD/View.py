from . import Operasi

def delete_console() :
    '''Membuat fungsi delet untuk menghapus data yang exist dalam database'''
    read_console() # membaca database dan menampilkannya
    while(True) :
        '''memilih data buku yang ingin di delete berdasarkan nomor buku'''
        print("Silahkan pilih nomor buku yang akan di delete")
        no_buku = int(input("Nomor Buku: "))
        data_buku = Operasi.read(index=no_buku)

        # untuk mengecek keberadaan buku ada atau tidak di dalam database
        if data_buku :
            # untuk memecah data
            data_break = data_buku.split(',')
            pk = data_break[0]
            data_add = data_break[1]
            penulis = data_break[2]
            judul = data_break[3]
            tahun = data_break[4][:-1] # fungsi [:-1] untuk membatasi pemecahan data dari data_buku sehingga tidak mengambil karakter paling akhir di dalam data_buku yaitu "\n"
        
            '''Untuk menampilkan data yang dipilih'''
            # data yang ingin didelete
            print("\n"+"="*100)
            print("Data yang ingin anda hapus")
            print(f"1. Judul\t: {judul:.40}")
            print(f"2. Penulis\t: {penulis:.40}")
            print(f"3. Tahun\t: {tahun:4}")

            # untuk memverifikasi data yang akan dihapus
            is_done = input("Apakah anda yakin (y/n) ? ")
            if is_done == "y" or is_done == "Y" :
                Operasi.delete(no_buku)
                break
            
        else :
            print("nomor tidak valid, silahkan masukan lagi")
    
    print("Data berhasil di hapus")

def update_console() :
    '''Membuat fungsi update terhadap data yang sudah exist di dalam database'''
    read_console() # membaca database dan menampilkannya
    while(True) :
        '''memilih data buku yang ingin di update berdasarkan nomor buku'''
        print("Silahkan pilih nomor buku yang akan di update")
        no_buku = int(input("Nomor Buku: "))
        data_buku = Operasi.read(index=no_buku)

        data_break = data_buku.split(',')
        pk = data_break[0]
        data_add = data_break[1]
        penulis = data_break[2]
        judul = data_break[3]
        tahun = data_break[4][:-1] # fungsi [:-1] untuk membatasi pemecahan data dari data_buku sehingga tidak mengambil karakter paling akhir di dalam data_buku yaitu "\n"

        # untuk mengecek keberadaan buku ada atau tidak di dalam database
        if data_buku :
            break
        else :
            print("nomor tidak valid, silahkan masukan lagi")

    while True :
        '''Untuk menampilkan data yang dipilih dan memberikan opsi untuk update isi konten dari data yang dipilih'''
        # data yang ingin diupdate
        print("\n"+"="*100)
        print("Silahkan pilih data apa yang ingin anda ubah")
        print(f"1. Judul\t: {judul:.40}")
        print(f"2. Penulis\t: {penulis:.40}")
        print(f"3. Tahun\t: {tahun:4}")

        # memilih mode untuk update
        user_option = input("Pilih data [1,2,3] : ")
        print("\n"+"="*100)
        match user_option :
            case "1" : judul = input("Judul\t: ") # untuk update judul
            case "2" : penulis = input("Penulis\t: ") # untuk update penulis
            case "3" : # untuk update tahun
                while True :
                    # untuk membuat inputan data tahun berupa angka dan apabila tidak akan disuruh masukkin datanya lagi hingga benar
                    try :
                        tahun = int(input("Tahun\t: "))
                        # untuk membatasi inputan angka pada tahun harus berjumlah 4 angka
                        if len(str(tahun)) == 4 :
                            break
                        else :
                            print("Tahun harus angka, silahkan masukan tahun lagi (yyyy)") 
                    except :
                        print("Tahun harus angka, silahkan masukan tahun lagi (yyyy)")
            case _: print("index tidak cuocuoook") # apabila opsi yang dipilih tidak sesuai dengan indexnya

        # menampilkan data hasil update
        print("Data baru anda")
        print(f"1. Judul\t: {judul:.40}")
        print(f"2. Penulis\t: {penulis:.40}")
        print(f"3. Tahun\t: {tahun:4}")

        is_done = input("Apakah data sudah sesuai (y/n) ? ")
        if is_done == "y" or is_done == "Y" :
            break
    Operasi.update(no_buku,pk,data_add,tahun,judul,penulis)

def create_console() :
    '''Membuat fungsi create untuk membuat data baru yang disimpan di database'''
    print("\n\n"+"="*100)
    print("Silahkan tambah data buku\n")

    penulis = input("Penulis\t: ")
    judul = input("Judul\t: ")
    while True :
        # untuk membuat inputan data tahun berupa angka dan apabila tidak akan disuruh masukkin datanya lagi hingga benar
        try :
            tahun = int(input("Tahun\t: "))
            # untuk membatasi inputan angka pada tahun harus berjumlah 4 angka
            if len(str(tahun)) == 4 :
                break
            else :
               print("Tahun harus angka, silahkan masukan tahun lagi (yyyy)") 
        except :
            print("Tahun harus angka, silahkan masukan tahun lagi (yyyy)")
    Operasi.create(tahun,judul,penulis) # untuk memasukkan data baru ke database
    print("\nBerikut adalah data baru anda")
    read_console() # untuk mengecek apakah data baru tersimpan ke database

def read_console() :
    '''Membuat fungsi Read Database dan menampilkannya melalui konsol'''
    data_file = Operasi.read()
    
    # styling dalam menampilkan data dari database
    index = "No"
    judul = "Judul"
    penulis = "Penulis"
    tahun = "Tahun"

    # Header
    print("\n"+"="*100)
    print(f"{index:4} | {judul:40} | {penulis:40} | {tahun:5}") # buat header
    print("-"*100)

    # Data
    for index, data in enumerate(data_file) :
        data_break = data.split(",")
        pk = data_break[0]
        date_add = data_break[1]
        penulis = data_break[2]
        judul = data_break[3]
        tahun = data_break[4]
        print(f"{index+1:4} | {judul:.40} | {penulis:.40} | {tahun:4}",end="") # arti .40 adalah memberikan spasi sebanyak jumlah karakter yang diperlukan agar jumlahnya sesuai contohnya judul karena sudah ada nama judulnya untuk menjadi jumlah karakternya 40 maka diperlukan spasi tambahan



    # Footer
    print("="*100+"\n")
