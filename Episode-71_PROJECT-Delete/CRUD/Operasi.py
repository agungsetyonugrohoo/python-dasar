...
def delete(no_buku) :
    ...
    try :
        with open(Database.DB_NAME, 'r') as file :
            counter = 0 # untuk menghitung readlines yang menyatakan posisi data apa yang akan kita delete
            while True :
                content = file.readline()
                if len(content) == 0 : # jika data kosong
                    break # berhenti apabila kosong datanya
                elif counter == no_buku - 1 : # jika content yang dituju sama dengan index buku yang ingin di delete
                    pass # menskip data yang ingin dihapus dan tidak memindahkannya ke temporary data
                else : # jika tidak memenuhi kedua kondisi di atas
                    with open("data_temp.txt", 'a', encoding="utf-8") as temp_file :
                        # sebagai data temporary untuk memindahkan data yang tidak ingin dihapus dari database
                        temp_file.write(content)
                counter += 1 # increment dalam membaca atau mengakses data dalam database
    except :
